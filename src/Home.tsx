...
interface Graficos {
  alumnosPorSemestre: Array<{ semestre: string; total: number }>;
  alumnosPorEspecialidad: Array<{ especialidad: string; total: number }>;
  alumnosPorGrupo: Array<{ grupo: string; total: number }>;
  distribucionCalificaciones: Array<{ rango: string; total: number }>;
  alumnosPorSexo: Array<{ sexo: string; total: number }>;
  docentesPorSexo: Array<{ sexo: string; total: number }>;
}
...
export const Home = () => {
  ...
  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/dashboard/estadisticas")
      .then((res) => res.json())
      .then((response) => {
        if (response.success) {
